...
(function ($, window) {
	...
	publicMethod.init = function () {
		...
		$overlay.click(function () {
			if (settings.overlayClose) {
				publicMethod.close();
				//alert('m3h3di');
				if( document.getElementById('wc_timer') )
					document.getElementById('wc_timer').style.display = 'block'; //edited by m3h3di
			}
		});
	...
	publicMethod.position = function (speed, loadedCallback) {
		...
		posTop = 10;	// edited by m3g3di static top position
	...
	publicMethod.load = function (launched) {		
		...
		$loadingOverlay.show();	// show the light box		
		if (settings.iframe) {
				...
				///========= Edited By MUKUL ==============
				var path = settings.href;
				var path = path.replace("pop_up.php?img_name=","");
				//alert(path);
				my_img = new Image();
				//alert(path);
				my_img.onload = function () {
					//settings.h = my_img.height + 780;
					//settings.w = my_img.width + 60;
					settings.h = my_img.height + 100;
					settings.w = my_img.width + 60;
					if( settings.w < 500 ) settings.w =500;
